...
module.exports = function (io, res) {
      ...
      if (err || !user) { res.send({ "status": "0", "message": res.__("No " + CONFIG.USER + " Found for this Email") }); } else {
        db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
          if (err || !settings) {
            ...
            newdata.verification_code = [{ "email": library.randomString(6, '#') }];
      ...
      if (err) {
        ...
        db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
                          if (err || !currencies) {
                            res.send({
                              "status": 0,
                              "message": res.__('Invalid Error, Please check your data')
                            });
                          } else {
        ...
        res.send({
          ...
          'distance_by' : settings.settings.distanceby,
          ...
          'google_api': CONFIG.GOOGLE_MAP_API_KEY_SERVER,
  ...
  controller.changeMobile = function (req, res) {
    ...
    db.GetOneDocument('users', { '_id': request.user_id }, {}, {}, function (err, CheckPhone) {
      if (err) {
        ...
        db.GetOneDocument('users', { '_id': { '$ne': request.user_id }, 'phone.code': req.body.country_code, 'phone.number': req.body.phone_number }, {}, {}, function (err, CheckotherPhone) {
          if (err || CheckotherPhone) {
      ...
      if (err) {
        ...
        db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
          if (err || !currencies) {
            ...
            db.GetDocument('users', { $or: [{ 'email': data.email }, { 'phone.number': data.email }] }, {}, {}, function (err, docs) {
              if (err || !docs[0]) {
                res.send({
                  "status": 0,
                  "message": res.__('Please check the email and try again')
                ...
                if (!docs[0].password || docs[0].password.length == 0) {
                  ...
                  if (validPassword(req.body.password, docs[0].password)) {
                    if (docs[0].status == 1) {
                      db.UpdateDocument('users', { $or: [ { 'email': data.email }, { 'phone.number': data.email }] }, { 'activity.last_login': new Date() }, {}, function (err, response) {
                        if (err || response.nModified == 0) {
                          res.send({
                            "status": 0,
                            "message": res.__('Please check the email and try again')
                          ...
                          if (data.token) {
                            db.UpdateDocument('users', { $or: [ { 'email': data.email }, { 'phone.number': data.email }] }, { 'device_info.device_type': 'ios', 'device_info.device_token': data.token, 'activity.last_login': new Date() }, {}, function (err, response) {
                              if (err || response.nModified == 0) {
                                res.send({
                                  "status": 0,
                                  "message": res.__('Please check the email and try again')
                            ...
                            db.UpdateDocument('users', { $or: [{ 'email': data.email }, { 'phone.number': data.email }] }, { 'device_info.device_type': 'android', 'device_info.gcm': data.gcm, 'activity.last_login': new Date() }, {}, function (err, response) {
                              if (err || response.nModified == 0) {
                                res.send({
                                  "status": 0,
                                  "message": res.__('Please check the email and try again')
                            ...
                            db.UpdateDocument('users', { 'email': data.email }, { 'device_info.device_type': 'ios', 'device_info.device_token': data.deviceToken, 'activity.last_login': new Date() }, {}, function (err, responseuser) {
                              if (err || responseuser.nModified == 0) {
                                ...
                                res.send({
                                  ...
                                  "user_id": insertdocID,
                                  "soc_key": bcrypt.hashSync(insertdocID, bcrypt.genSaltSync(8), null),
                                  "user_name": usermailrefer.username,
                                  "user_group": "User",
                                  "email": response.email,
                                  "country_code": response.phone.code,
                                  "phone_number": response.phone.number,
  ...
  controller.bookJob = function (req, res) {
    ...
    var errors = [];
    req.checkBody('user_id', CONFIG.USER + ' ID is Required').optional();
    ...
    req.checkBody('service_address', 'Service Address is Required').notEmpty();
    //req.checkBody('ratetype', 'Enter ratetype ').notEmpty();
    ...
    //data.reach_time = req.body.ratetype ? ratetype : "";
    db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
      if (err) {
        res.send({
          "status": "0",
       ...
       /*  if ((data.reach_date == today) && (hour == currenthour)) {
          ...
          var defaultCondition = [{
            "$geoNear": {
              ...
              spherical: true,
              maxDistance: kmdistance / distanceval
            }
          },
          {
            "$redact": {
          ...
          },
          {
            $project:
            {
             ...
             /*  'current_task_check' :
              {
                $cond: { if: { 'current_task' : { $exist : true} }, then: { 'task_date' : {$ne : 'current_task_date'}}, else: { 'current_task' : { $exist : false}} }
              } */
            }
          },
          {
        ...
        /*   { "$lookup": { from: "experience", localField: "taskerskills.experience", foreignField: "_id", as: "experience" } }, */
          {
            "$group":
            {
              "_id": "$_id",
                      ...
                      db.GetAggregation('tasker', taskercondition, function (err, docdata) {
                        if (err || docdata.length == 0) {
                          ...
                          db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
                            ...
                            data.hourly_rate = 0;
                            data.service_address = req.body.service_address;
                            ...
                            data.booking_information.booking_date = timezone.tz(formatedDate, settings.settings.time_zone).utc().format();
                            data.booking_information.service_type = category.name;
                            data.booking_information.work_type = category.name;
                                    ...
                                    if (err) {
                                      res.send(err);
                                          ...
                                          for (var i = 0; i < docdata[0].taskers.length; i++) {
                                            ...
                                            providertemp.currency_symbol = currencies.symbol;
                                          ...
                                          if (lowest == highest) {
                                            lowest = 0;
                                              ...
                                              res.send({ booking_id: bookdata.booking_id, task_id: bookdata._id, count: docdata[0].count, status: "1", lowesthourlyrate: lowest, highesthourlyrate: highest, response: providerList , categorytype : category.ratetype , categorytypestatus : category.ratetype == 'Flat' ? 1  : 2 });
                                    }
                                  });
                                }
                              });
  ...
  controller.mapbookJob = function (req, res) {
    var errors = [];
    req.checkBody('user_id', res.__(CONFIG.USER + ' ID is Required')).optional();
        ...
        if (req.body.category) {
          var taskercondition = [{
            "$geoNear": {
              ...
              distanceField: "distance",
              includeLocs: "location",
              query: { "status": 1, "availability": 1, "taskerskills": { $elemMatch: { childid: new mongoose.Types.ObjectId(categoryid), status: 1 } }, "working_days": working_days /* "current_task": { $exists: false } */ },
              distanceMultiplier: distanceval,
              spherical: true
            }
          },
          { "$redact": { "$cond": { "if": { "$lte": ["$distance", settings.settings.tasker_distance] }, "then": "$$KEEP", "else": "$$PRUNE" } } },
          ...
          var taskercondition = [{
            "$geoNear": {
              ...
              distanceField: "distance",
              includeLocs: "location",
              query: { "status": 1, "availability": 1, "taskerskills": { $elemMatch: { categoryid: new mongoose.Types.ObjectId(maincategoryid), status: 1 } }, "working_days": working_days /* "current_task": { $exists: false } */ },
              distanceMultiplier: distanceval,
              spherical: true
            }
          },
          ...
          var taskercondition = [{
            "$geoNear": {
              ...
              distanceField: "distance",
              includeLocs: "location",
              query: { "status": 1, "availability": 1, "working_days": working_days /* "current_task": { $exists: false } */ },
              distanceMultiplier: distanceval,
              spherical: true
            }
          },
                    ...
                    res.send({
                      ...
                      for (var i = 0; i < docdata[0].taskers.length; i++) {
                        ...
                        if (req.body.category) {
                          ...
                          providertemp.ratetype = docdata[0].taskers[i].taskerskills.filter(function (ol) {
                            return ol.childid == req.body.category;
                          }).map(function (ma) {
                            return ma;
                          })[0].ratetype || "";
                      ...
                      res.send({ count: docdata[0].count, status: "1", response: providerList , distance : settings.settings.tasker_distance });
                }
              });
        } else {
          res.send({ count: 0, result: [] });
                  ...
                  db.GetCount('tasker', condition, function (err, count) {
                    if (err) {
                      ...
                      for (var i = 0; i < docdata.length; i++) {
                        ...
                        providertemp.name = docdata[i].username;
  ...
  console.log("userBooking");
    ...
    db.GetOneDocument('users', { _id: req.body.user_id }, {}, {}, function (usererr, userdata) {
      if (usererr || !userdata) {
        ...
        db.GetOneDocument('task', { _id: req.body.taskid }, {}, options, function (err, taskdatas) {
          if (err || !taskdatas) {
            res.send({
              "status": "0",
            ...
            var taskdata = JSON.parse(JSON.stringify(taskdatas));
            ...
            db.GetOneDocument('tasker', { _id: req.body.taskerid }, {}, options, function (taskerErr, taskerData) {
              if (taskerErr || !taskerData) {
                ...
                console.log("taskerData.availability",taskerData.availability)
                if(taskerData.availability !=  1){
                  res.send({
                    "status": 0,
                    "response": res.__("Tasker Not Availale")
                  });
                  return;
                }
                ...
                for (var key = 0; key < taskerData.taskerskills.length; key++) {
                  if (taskerData.taskerskills[key].childid == catid) {
                    ...
                    if(taskerData.taskerskills[key].ratetype){
                      var ratetype = taskerData.taskerskills[key].ratetype == 'Flat' ? 1 : 2;
                    }else if(taskdata.category.ratetype) {
                      var ratetype = taskdata.category.ratetype == 'Flat' ? 1 : 2;
                    }else{
                      var ratetype = 2; //default hourly
                    }
                    console.log("ratetype", taskerData.taskerskills[key]);
                ...
                db.GetOneDocument('users', { _id: req.body.user_id }, {}, {}, function (useraddresserr, useraddreessdata) {
                  if (useraddresserr || !useraddreessdata) {
                    ...
                    GoogleAPI.geocode(geocode, function (response) {
                      ...
                      taskdata.task_address = taskdata.task_address.exactaddress,
                      taskdata.tasker = req.body.taskerid;
                      taskdata.user = req.body.user_id;
                      taskdata.status = 1;
                      taskdata.hourly_rate = rate;
                      taskdata.ratetype = ratetype;
                      taskdata.bookingmode = "booklater";
                      var reach_date = timezone.tz(new Date(), CONFIG.TIMEZONE).utc().format();
                      var est_reach_date = timezone.tz(new Date(), CONFIG.TIMEZONE).utc().format();
                      ...
                      db.UpdateDocument('task', { _id: req.body.taskid }, taskdata, {}, function (err, docdata) {
                        ...
                        if (err) {
                          ...
                          var job_booking_time = timezone.tz(new Date(), CONFIG.TIMEZONE).utc().format();
                          db.UpdateDocument('task', { _id: req.body.taskid }, { 'history.job_booking_time': job_booking_time }, {}, function (err, docdata) {
                            if (err) {
                              res.send({
                                "status": "0",
                                "response": res.__("please check task")
  ...
  controller.forgotPassword = function (req, res) {
    ...
    async.waterfall([
      ...
      mailData.html.push({ name: 'url', value: settings.settings.site_url + 'reset-password/user' + '/' + user._id + '/' + user.reset_code });
      mailcontent.sendmail(mailData, function (err, response) { });
      var to = user.phone.code + user.phone.number;
            ...
            if (err || !user) {
              ...
              db.GetOneDocument('task', { 'booking_id': req.body.job_id }, {}, extension, function (err, bookings) {
                if (err || !bookings) {
                  ...
                  review_opt_arr = [{
                    'option_holder': 'user',
                    'option_name': res.__('Way Of Approach'),
                    'status': 'Active',
                    'option_id': 2,
                    'image': userimg,
                    ...
                    GoogleAPI.geocode(geocode, function (response) {
                      ...
                      res.send({
                        ...
                        "response": {
                          ...
                          "provider_name": proRespo[0].username,
  ...
  controller.viewJob = function (req, res) {
    ...
    async.waterfall([
      ...
      if (tasker.provider_location) {
        provider_location = tasker.provider_location;
      }
      else {
        provider_location = {
          'provider_lng': tasker.location.lng,
          'provider_lat': tasker.location.lat
        };
      }
      ...
      switch (task.status.toString()) {
        case '1':
          request.status = res.__('Onprogress');
          request.btn_group = 1;
          break;
        case '2':
          ...
          break;
        case '3':
          request.status = res.__('StartOff');
          request.btn_group = 3;
          break;
        case '4':
          ...
          break;
        case '5':
          request.status = res.__('StartJob');
          request.btn_group = 5;
          break;
        case '6':
          request.status = res.__('Request Payment');
          request.btn_group = 6;
          break;
        case '7':
      ...
      var jobsArr = {
        ...
        'service_address': task.service_address,
      ...
      var timelineArr = {
        ...
        "job_booking_time": res.__('Job booked'),
        "provider_assigned": res.__('Hired job'),
        "provider_start_off_time": res.__('Start Off'),
        "location_arrived_time": res.__(CONFIG.TASKER + ' arrived at job location'),
        "job_started_time": res.__('Job has been started'),
        "job_completed_time": res.__('Job has been completed'),
        "job_closed_time": res.__('Payment has been completed'),
        "wallet_usage_time": res.__('Payment made through wallet'),
        "job_cancellation_time": res.__('Job has been cancelled')
      };
      var job_history = {};
      if (task.history) {
            ...
            db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
              if (err || !settings) {
                ...
                if (bookRespo.status <= 5) {
                  ...
                  var jobDetails = {};
                  var job_cancellation_time = timezone.tz(new Date(), CONFIG.TIMEZONE).utc().format();
                  jobDetails = {
                    'cancellation': {
                      'reason': req.body.reason,
                      'type': 'user',
                      'date': job_cancellation_time,
                      'status': 1
                    },
                    'usertaskcancellationreason': req.body.reason,
                    'status': 8,
                    'history.job_cancellation_time': job_cancellation_time
                  };
        ...
        async.parallel({
          ...
          if (err || !result || !result.task) {
            ...
            db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
                ...
                for (var i = 0; i < result.paymentgateway.length; i++) {
                  if (result.paymentgateway[i].gateway_name == "Stripe" && result.paymentgateway[i].status == 1) {
                    ...
                    paymentArr.push({
                      ...
                      'code': res.__(result.paymentgateway[i].alias),
              ...
              /*   if (result.task.invoice.amount.minimum_cost >= 0) {
                ...
                if (result.task.invoice.worked_hours_human) {
                  var response = { 'title': res.__('Total Hours Worked'), 'dt': '0', 'amount': result.task.invoice.worked_hours_human };
                  billing.push({ response });
                }
            ...
            billing.push({ response });
              ...
              var rate = result.task.hourly_rate;
              result.task.tasker.taskerskills.forEach(function(el) {
                if(JSON.stringify(el.childid) == JSON.stringify(result.task.category._id)) {
                  rate = el.hour_rate;
                }
              })
                if (result.task.hourly_rate && result.task.category.ratetype == 'Flat') {
                  var response = { 'title': res.__('Fixed Rate'), 'dt': '1', 'amount': (rate * currencies.value).toFixed(2) };
                  billing.push({ response });
                }
                else{
                  if(result.task.hourly_rate){
                    var response = { 'title': res.__('Houlry Rate'), 'dt': '1', 'amount': (rate * currencies.value).toFixed(2) };
                    billing.push({ response });
                  }
                }

                if (result.task.invoice.amount.discount > 0) {
                  var response = { 'title': res.__('Coupon discount'), 'dt': '1', 'amount': (result.task.invoice.amount.discount * currencies.value).toFixed(2) };
                  billing.push({ response });
                }


                if (result.task.invoice.amount.total) {
                  var response = { 'title': res.__('Task amount'), 'dt': '1', 'amount': (result.task.invoice.amount.total * currencies.value).toFixed(2) };
                  billing.push({ response });
                }

                if (result.task.invoice.amount.service_tax) {
                  var response = { 'title': res.__('Service tax'), 'dt': '1', 'amount': (result.task.invoice.amount.service_tax * currencies.value).toFixed(2) };
                  billing.push({ response });
                }

                if (result.task.invoice.amount.wallet_usage > 0) {
                  var response = { 'title': res.__('Wallet used amount'), 'dt': '1', 'amount': (result.task.invoice.amount.wallet_usage * currencies.value).toFixed(2) };
                  billing.push({ response });
                }
                if (result.task.invoice.amount.paid_amount > 0) {
                  var response = { 'title': res.__('Paid amount'), 'dt': '1', 'amount': (result.task.invoice.amount.paid_amount * currencies.value).toFixed(2) };
                  billing.push({ response });
                }
                if (result.task.invoice.amount.extra_amount) {
                  var response = { 'title': res.__('Miscellaneous amount'), 'dt': '1', 'amount': (result.task.invoice.amount.extra_amount * currencies.value).toFixed(2) };
                  billing.push({ response });
                }
                if (result.task.invoice.amount.grand_total) {
					// var extra_amount = result.task.invoice.amount.extra_amount ? result.task.invoice.amount.extra_amount : 0;
					// var discount =  result.task.invoice.amount.discount  ? result.task.invoice.amount.discount : 0;
                  var response = { 'title': res.__('Grand Total'), 'dt': '1', 'amount': (result.task.invoice.amount.grand_total * currencies.value).toFixed(2) };
                  billing.push({ response });
                }
                ...
                if (result.task.invoice.amount.balance_amount != 0 || result.task.invoice.amount.balance_amount != "") {
                  if (result.task.payment_type == "wallet-other") {
                    var response = { 'title': res.__('Balance Total'), 'dt': '1', 'amount': (result.task.invoice.amount.balance_amount * currencies.value).toFixed(2) };
                    billing.push({ response });
                  }
                ...
                if (result.task.payment_type) {
                  var response = { 'title': res.__('Payment mode'), 'dt': '0', 'amount': payment_type };
                  billing.push({ response });
                }
                res.send({
                  "status": "1",
              ...
              if (err || !currencies) {
                ...
                var finaldata = 60 - (parseFloat(datecheck[0]) * 60);
                  ...
                  if (bookings.invoice.amount.total) {
                    var response = { 'title': 'Task amount', 'dt': '1', 'amount': (bookings.invoice.amount.total * currencies.value).toFixed(2) };
                    data.response.job.billing.push(response);
                  }
  ...
  controller.submitRattings = function (req, res) {
    ...
    console.log("qwertyuighjkl,req.body",req.body)
    var errors = [];
                      ...
                      if (err) {
                        ...
                        db.GetAggregation('tasker', getQuery, function (err, docdata) {
                          if (err) {
                            ...
                            db.GetDocument('tasker', { _id: provider_id }, {}, {}, function (err, taskerDetails) {
                              if (err) {
                                  res.send(err);
                              } else {
              ...
              db.GetAggregation('messages', [
                ...
                if (err || messages.length == 0) {
                  ...
                  for (var i = 0; i < messages.length; i++) {
                    var message = {};
                    message.task_id = messages[i]._id;
                    message.user_name = messages[i].user_name;
  ...
  controller.updateuserlanguage = function (req, res) {
    console.log("hhhhhhhhhhhhhhhhhhhhhhhhhhhhhh",req.body)
    var data = {};
    data.status = 0;

    req.checkBody('userid', res.__('UserId  is required')).notEmpty();
    req.checkBody('langcode', res.__('langcode is invalid')).notEmpty();
    ...
    db.UpdateDocument('users', { '_id': request.userid }, { 'language': request.langcode }, {}, function (err, response) {
      if (err || response.nModified == 0) {
