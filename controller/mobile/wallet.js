...
module.exports = function(io, i18n) {
                            ...
                            if (err) {
                                ...
                                async.waterfall([
                                    ...
                                    function(settings, token, callback) {
                                        ...
                                        charge.description = res.__('Wallet Recharge');
    ...
    controller.mpesaProcess = function(req, res) {
        var data = {};
        data.status = '0';
        ...
        db.walletReacharge({ status: { $ne: 0 }, alias: 'stripe' }, {}, {}, function(err, paymentgateway) {
            ...
            if (err) {
                ...
                request.user_id = { 'user_id': new objectID(req.body.user_id) }
                request.total_amount = { 'amount': new objectID(req.body.total_amount) }

                request.phone_number = { "phone": new objectID(req.body.phone_number) }

                db.walletReacharge('users', { '_id': request.user_id }, {}, {}, function(err, user) {
                    if (err || !user) {
                        //res.redirect("http://" + req.headers.host + '/mobile/mobile/failed');
                        ...
                        db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function(err, currencies) {
                            if (err) {
                                //res.redirect("http://" + req.headers.host + '/mobile/mobile/failed');
                                data.error = 'Unable to set the default currency';
                                data.status = 201;
                                res.send(data);
                                return;
                            } else {
                                ...
                                async.waterfall([
                                    ...
                                    function(settings, callback) {
                                        if (!request.transaction_id) {
                                            ...
                                            db.walletReacharge('transaction', transaction, function(err, transaction) {
                                                request.transaction_id = transaction._id;
                                                request.trans_id = transaction._id;
                                                request.trans_date = transaction.createdAt;
                                                request.avail_amount = transaction.amount;
                                    ...
                                    if (err) {
                                        //res.redirect("http://" + req.headers.host + '/mobile/mobile/failed');
                                        data.error = 'Unable to save transaction to the database';
                                        data.status = 201;
                                        res.send(data);
                                        return;
                                        ...
                                        userLibrary.walletRecharge({ 'user': request.user_id, 'transaction': request.transaction_id }, function(err, response) {
                                            if (err || !response) {
                                                res.send(err);
                                            } else {
                                                //res.redirect("http://" + req.headers.host + '/mobile/payment/pay-completed/bycard');
                ...
                async.waterfall([
                    ...
                    function(paymentgateway, settings, template, callback) {
                        db.GetOneDocument('task', { '_id': request.task }, {}, {}, function(err, task) {
                            ...
                            if (err || !task) {
                                data.response = res.__('task is completed');
                                res.send(data);
    ...
    controller.applyCoupontest = function(req, res) {
        ...
        req.checkBody('booking_id', res.__('Booking_id is Required')).notEmpty();
            ...
            if (err || coupondata.length == 0) {
                ...
                db.GetDocument('coupon', { status: { $ne: 0 }, code: req.body.code, "expiry_date": { "$gte": isodate }, "valid_from": { "$lte": isodate } }, {}, {}, function(err, data) {
                    if (err || data.length == 0) {
                        res.send({ "status": "0", "response": res.__("Coupon Code Date Expired") });
                    } else {
                            ...
                            if (err || !taskdata) {
                                ...
                                db.GetDocument('coupon', { status: { $ne: 0 }, code: req.body.code }, {}, {}, function(err, couponlimit) {
                                    if (err || couponlimit.length == 0) {
                                        res.send({ "status": "0", "response": res.__("Coupon Code Limit Exceed") });
                                    } else {
                                        db.GetAggregation('task', [
                                                            ...
                                                            if (err || !currencies) {
                                                                ...
                                                                db.GetDocument('task', { 'booking_id': req.body.booking_id, 'invoice.coupon': { $exists: false } }, {}, {}, function(err, taskdata) {
                                                                    if (err || !taskdata || taskdata.length == 0) {
                                                                        res.send({ "status": "0", "response": res.__("Coupons is Already Applied") });
                                                                    } else {
                                                                        db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function(err, settings) {
                                                                            if (err || !settings) {
                                                                                res.send({ "status": "0", "response": res.__("No settings Found") });
                                                                            } else {
                                                                                var invoice = {};
                                                                                invoice.amount = {};
                                                                                var discount = 0.00;
                                                                                ...
                                                                                var servicetax = (parseInt(settings.settings.service_tax) / 100) * (taskdata[0].invoice.amount.total - (discount || 0));
                                                                                var grand_total = temporaryprice - discount + servicetax;
                                                                                ...
                                                                                var update = { 'invoice.amount.coupon': discount, 'invoice.coupon': req.body.code, 'invoice.amount.discount': discount, 'invoice.amount.balance_amount': balance_amount, 'invoice.amount.service_tax': servicetax, 'invoice.amount.grand_total': grand_total };

                                                                                db.UpdateDocument('task', { _id: new mongoose.Types.ObjectId(taskdata[0]._id) }, update, function(err, result) {
                                                                                    if (err || result.nModified == 0) {
                                                                                        res.send({ "status": "0", "response": res.__("Error In Coupon updation") });
                                                                                    } else {
                                                                                        ...
                                                                                        db.UpdateDocument('coupon', { status: { $ne: 0 }, code: req.body.code }, { 'usage.total_coupons': result }, function(err, result) {
                                                                                            if (err || result.nModified == 0) {
                                                                                                ...
                                                                                                res.send({ "status": "1", "response": res.__("Coupon used successfully"), "discount": (discount * currencies.value).toFixed(2) });
                                                                                            }
                                                                                        });
                                                                                    }
                                                                                });
                    ...
                    if (err) {
                        ...
                        async.waterfall([
                            ...
                            function(paymentgateway, transaction, settings, callback) {
                                db.GetDocument('emailtemplate', { name: { $in: ['PaymentDetailstoAdmin', 'PaymentDetailstoTasker', 'PaymentDetailstoUser'] }, 'status': { $ne: 0 } }, {}, {}, function(err, template) {
                                    if (err || !template) {
                                        data.response = res.__('Unable to get email template');
                                        res.send(data);
                                    } else { callback(err, paymentgateway, transaction, settings, template); }
                                });
                            }
                            ...
                            if (err) {
                                ...
                                json.transactions[0].amount.currency = currencies.code;
                                json.redirect_urls.return_url = "http://" + req.headers.host + "/mobile/app/wallet-recharge/mob-execute?user=" + request.user + "&transaction=" + transaction._id + "&lang=" + lang;
                                json.redirect_urls.cancel_url = "http://" + req.headers.host + "/checkout/payment/paypal/cancel?user=" + request.user + "&transaction=" + transaction._id + "&lang=" + lang;
                                paypal.payment.create(json, function(error, payment) {
                                    if (error) {
                                        res.send({
                                            "status": "0",
                                ...
                                if (result.state != 'approved') {
                                    ...
                                    userLibrary.walletRecharge({ 'user': request.user, 'transaction': transaction._id, 'gateway_response': result }, function(err, response) {
                                        if (err || !response) {
                                            ...
                                            res.redirect("http://" + req.headers.host + '/mobile/mobile/paypalsucess?lang=' + lang);
                                        }
                                    });
                                }
                            }
