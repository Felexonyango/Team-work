module.exports = function(io, i18n) {
                        ...
                        if (request.deviceToken) {
                          db.UpdateDocument('tasker', {
                            ...
                            if (err || response.nModified == 0) {
                              ...
                              if (user[0].avatar) {
                                data.response.provider_image = GLOBAL_CONFIG.site_url + user[0].avatar;
                              } else {
                                data.response.provider_image = GLOBAL_CONFIG.site_url + CONFIG.USER_PROFILE_IMAGE_DEFAULT;
                              }
                              data.response.provider_name = user[0].username;
                  ...
                  db.GetOneDocument('settings', {
                    ...
                    if (err) {
                      ...
                      for (var i = 0; i < bookings.length; i++) {
                        ...
                        job.location_lng = bookings[i].location.lng || '';
    ...
    controller.savebankingInfo = function(req, res) {
      ...
      req.checkBody('account_type', res.__('Invalid Account Type')).notEmpty();
      req.checkBody('bank_name', res.__('Invalid Bank Name')).optional();
      req.checkBody('branch_name', res.__('Invalid Branch Name')).optional();
      req.checkBody('branch_address', res.__('Invalid Branch Address')).optional();
      req.checkBody('swift_code', res.__('Invalid Swift Code')).optional();
      req.checkBody('routing_number', res.__('Invalid Routing Number')).notEmpty();
      //validation
      ...
      req.sanitizeBody('account_type').trim();
                ...
                if (err || !currencies) {
                  ...
                  if (providers) {
                    ...
                    GoogleAPI.geocode(geocode, function(response) {
                      ...
                      data.status = '1';
                      data.response = {};
                      data.response.provider_name = providers.username;
      ...
      async.waterfall([
        function(callback) {
          db.GetOneDocument('tasker', {
            ...
            if (err || !user) {
              data.response = res.__('No ' + CONFIG.TASKER + ' Found for this Phone or Email');
              res.send(data);
            } else {
              callback(err, user);
            }
      ...
      db.GetDocument('category', {
        ...
        if (err) {
          ...
          for (var i = 0; i < subcategories.length; i++) {
            ...
            subcats.ratetype = subcategories[i].ratetype;
              ...
              db.GetOneDocument('settings', {
                ...
                if (err || !settings) {
                  ...
                  if (bookings.status == 1 || bookings.status == 2 || bookings.status == 3 || bookings.status == 4) {
                    ...
                    cancelData.date = job_cancellation_time;
                    cancelData.status = 1;
                    ...
                    db.UpdateDocument('task', {
                      ...
                      'usertaskcancellationreason': request.reason,
                      'status': 8,
                      ...
                      if (err || response.nModified == 0) {
                        ...
                        mailData.html.push({
                          name: 'taskername',
                          value: bookings.tasker.username || ""
                        });
                        mailData.html.push({
                          name: 'taskname',
                      ...
                      for (var i = 0; i < bookings.length; i++) {
                        ...
                        switch (bookings[i].status) {
                          ...
                          case 2:
                            ...
                            break;
                          case 3:
                            request.job_status = res.__('StartOff');
                            request.btn_group = 3;
                            break;
                          case 4:
                            ...
                            break;
                          case 5:
                            request.job_status = res.__('StartJob');
                            request.btn_group = 5;
                            break;
                          case 6:
                            request.job_status = res.__('Request Payment');
                            request.btn_group = 6;
                            break;
                          case 7:
    ...
    controller.viewJob = function(req, res) {
      ...
      async.waterfall([
        ...
        data.response.job.distance_by = settings.settings.distanceby;
              ...
              db.GetOneDocument('task', {
                ...
                if (existingbookings) {
                  ...
                  if (bookings.status == 1) {
                    ...
                    // request.history.job_booking_time = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
                    // request.history.job_booking_time = timezone.tz(request.history.job_booking_time, CONFIG.TIMEZONE);
                    request.history.job_booking_time = timezone.tz(new Date(), settings.settings.time_zone).utc().format();
                    console.log("CONFIG.TIMEZONEmmmmmmmmmmmmmmmmmmmmm",settings.settings.time_zone)
                    //request.history.provider_assigned = new Date();
                    var formatedDate = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
                    // request.history.provider_assigned = timezone.tz(formatedDate,settings.settings.time_zone);
                    request.history.provider_assigned = timezone.tz(new Date(), settings.settings.time_zone).utc().format();
                    request.jobDetails = {};
                    ...
                    async.parallel({
                      ...
                      if (err) {
                        ...
                        mailData.html.push({
                          name: 'taskname',
                          value: bookings.booking_information.service_type || ""
                        });
                        mailData.html.push({
                          name: 'startdate',
                          value: job_date || ""
                        ...
                        mailData.html.push({
                          name: 'description',
                          value: bookings.booking_information.instruction || ""
                        });
                        mailData.html.push({
                          name: 'site_url',
                          value: result.settings.settings.site_url || ""
                        });
                        mailData.html.push({
                          name: 'site_title',
                          value: result.settings.settings.site_title || ""
                        });
                        mailData.html.push({
                          name: 'logo',
                          value: result.settings.settings.logo
                        });
                        mailcontent.sendmail(mailData, function(err, response) {});
                        ...
                        data.response.message = res.__('Job Accepted');
    ...
    controller.startOff = function(req, res) {
      ...
      req.checkBody('provider_lon', res.__('Enter Valid ' + CONFIG.TASKER + '_lng')).notEmpty();
      req.checkBody('provider_lat', res.__('Enter Valid ' + CONFIG.TASKER + '_lat')).notEmpty();
      //validation
          ...
          db.GetOneDocument('task', {
            ...
            if (err || !bookings) {
              ...
              if (bookings.status == 2) {
                console.log( " CONFIG.TIMEZONEjjjjjjjjjjjjjjjjjjjjjjj",CONFIG.TIMEZONE)
                var startOfftime = timezone.tz(new Date(), settings.settings.time_zone).utc().format();
                // var startOfftime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
                // startOfftime = timezone.tz(startOfftime, CONFIG.TIMEZONE);

                db.UpdateDocument('task', {
                  'booking_id': request.job_id
                }, {
                  'status': 3,
                  'history.provider_start_off_time': startOfftime
                }, {}, function(err, response) {
              ...
              db.GetOneDocument('settings', {
                ...
                if (err) {
                  ...
                  if (bookings.status == 3) {
                    // var arrivedtime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
                    // arrivedtime = timezone.tz(arrivedtime, CONFIG.TIMEZONE);
                    var arrivedtime = timezone.tz(new Date(), settings.settings.time_zone).utc().format();
                    db.UpdateDocument('task', {
                      'booking_id': request.job_id
                    }, {
                      'status': 4,
                      'history.location_arrived_time': arrivedtime
                    }, {}, function(err, response) {
                      if (err || response.nModified == 0) {
                              ...
                              db.GetCount('review', {
                                ...
                                if (err) {
                                  ...
                                  if (bookings.tasker && (bookings.status == 6 || bookings.status == 3 || bookings.status == 7)) {
									...
									console.log("bookings.category.ratetype , bookings.hourly_rate ", bookings.category.ratetype , bookings.hourly_rate)
                                      var billing = {
                                        'title': res.__('Fixed Rate'),
                                        'dt': '1',
									...
									console.log("bookings.hourly_rate ", bookings.hourly_rate)
                                    ...
                                    if (bookings.invoice.amount.extra_amount) {
                                      var billing = {
                                        'title': res.__('Miscellaneous amount'),
                                        'dt': '1',
                                    ...
                                    //     'title': 'Grand Total',
                                    //     'dt': '1',
                                    //     'amount': (bookings.invoice.amount.grand_total * currencies.value).toFixed(2)
                                    //   };
                                    //   data.response.job.billing.push(response);
                                    // }
                                    ...
                                    if (bookings.invoice.amount.total && bookings.invoice.amount.discount) {
                                      ...
                                      // var amount_final = bookings.invoice.amount.grand_total - bookings.invoice.amount.admin_commission;
                                      var lastamount_final = (bookings.invoice.amount.total - bookings.invoice.amount.admin_commission) + (bookings.invoice.amount.extra_amount ? bookings.invoice.amount.extra_amount : 0 );
                                      // var lastamount_final = amount_final - bookings.invoice.amount.service_tax;
                                      var finalCoupon = lastamount_final;
                                      var response = {
                                        'title': res.__('Tasker Earnings'),
                                        'dt': '1',
                          ...
                          if (!err) {
                            ...
                            db.GetDocument('emailtemplate', {
                              ...
                              if (err) {
                                ...
                                html = html.replace(/{{useraddress}}/g, bookings.service_address ? bookings.service_address : bookings.task_address);
                                ...
                                html2 = html2.replace(/{{useraddress}}/g, bookings.service_address ? bookings.service_address : bookings.task_address);
                                ...
                                html3 = html3.replace(/{{useraddress}}/g, bookings.service_address ? bookings.service_address : bookings.task_address);
    ...
    controller.jobTimeline = function(req, res) {
      ...
      request.timelineArr = [
        ...
        //   'value': CONFIG.NOTIFICATION.JOB_BOOKED
        // },
        // {
        //   'key': 'provider_assigned',
        //   'value': CONFIG.NOTIFICATION.HIRED_JOB
        // },
        // {
        //   'key': 'provider_start_off_time',
        //   'value': CONFIG.NOTIFICATION.PROVIDER_START_OFF_FROM_HIS_LOCATION
        // },
        // {
        //   'key': 'location_arrived_time',
        //   'value': CONFIG.NOTIFICATION.PROVIDER_ARRIVED_ON_JOB_LOCATION
        // },
        // {
        //   'key': 'job_started_time',
        //   'value': CONFIG.NOTIFICATION.JOB_HAS_BEEN_STARTED
        // },
        // {
        //   'key': 'job_completed_time',
        //   'value': CONFIG.NOTIFICATION.JOB_HAS_BEEN_COMPLETED
        // },
        // {
        //   'key': 'job_closed_time',
        //   'value': CONFIG.NOTIFICATION.JOB_HAS_BEEN_CLOSED
        // },
        // {
        //   'key': 'job_cancellation_time',
        //   'value': CONFIG.NOTIFICATION.JOB_HAS_BEEN_CANCELLED
        // }
        { 'key': 'job_booking_time', 'value': res.__('Job booked') },
            { 'key': 'provider_assigned', 'value': res.__('Hired job') },
            { 'key': 'provider_start_off_time', 'value': res.__('Start Off') },
            { 'key': 'location_arrived_time', 'value': res.__('Tasker Arrived') },
            { 'key': 'job_started_time', 'value': res.__('Job has been started') },
            { 'key': 'job_completed_time', 'value': res.__('Job has been completed') },
            { 'key': 'job_closed_time', 'value': res.__('Payment has been completed') },
            { 'key': 'job_cancellation_time', 'value': res.__('Job has been cancelled') }
      ];
      db.GetOneDocument('tasker', {
        '_id': request.provider_id
      }, {}, {}, function(err, provider) {
          ...
          db.GetOneDocument('task', {
            ...
            if (err || !bookings) {
              ...
              if (bookings.status == 6) {
                ...
                // var job_closed_time = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
                // job_closed_time = timezone.tz(job_closed_time, results.settings.time_zone);
                var job_closed_time = timezone.tz(new Date(), CONFIG.TIMEZONE).utc().format();
                var pay_summary = 'Cash';
                var paymentInfo = {
                  'invoice.status': 1,
                  status: 7,
                  'history.job_closed_time': job_closed_time
                };
                var transactions = {};
                var paymenttype;
                if (bookings.payment_type == "wallet-other") {
    ...
    controller.registerStp2 = function(req, res) {
      ...
      req.checkBody('lat', res.__('Invalid Lat')).notEmpty();
      req.checkBody('long', res.__('Invalid Long')).notEmpty();
      ...
      req.checkBody('work_lat', res.__('Lat is required')).notEmpty();
      req.checkBody('work_long', res.__('Long is required')).notEmpty();
      req.checkBody('line1', res.__('Invalid address')).optional();
      req.checkBody('line2', res.__('Invalid address')).optional();
      req.checkBody('city', res.__('City is required')).optional();
      req.checkBody('state', res.__('Zipcode is required')).optional();
    ...
    controller.taskerregister = function(req, res) {
      ...
      req.checkBody('username', res.__('Invalid Username')).notEmpty();
      req.checkBody('password', res.__('Invalid Password')).notEmpty();
      req.checkBody('working_days', res.__('Invalid Working Days')).notEmpty();
      req.checkBody('taskerskills', res.__('Invalid Tasker Skills')).notEmpty();
      req.checkBody('profile_details', res.__('Invalid Profile Details')).notEmpty();
      req.checkBody('birthdate', res.__('Invalid Birth Date')).notEmpty();
      req.checkBody('lat', res.__('Invalid Lat')).notEmpty();
      req.checkBody('long', res.__('Invalid Long')).notEmpty();
      req.checkBody('address', res.__('Invalid Address')).notEmpty();
      req.checkBody('availability_address', res.__('Invalid Work Address')).notEmpty();
      req.checkBody('location_lng', res.__('Lat is required')).notEmpty();
      req.checkBody('location_lat', res.__('Long is required')).notEmpty();
      ...
      var tasker = {};
      tasker.username = req.body.username;
      tasker.password = bcrypt.hashSync(req.body.password, bcrypt.genSaltSync(8), null);
      tasker.email = req.body.email;
      tasker.phone = {
        "code": req.body.phone_code,
        "number": req.body.phone_number
      };
      tasker.address = req.body.address;
      tasker.tasker_area = {
        'lng': req.body.long,
        'lat': req.body.lat
      };
      ...
      tasker.avatar = '';
      if (req.body.image) {
        var image = library.splitURL(url);
        tasker.avatar = image.pathname.substr(1);
      }

      tasker.gender = req.body.gender;

      var birthday = {};
      birthday.year = moment(req.body.birthdate, "MM-DD-YYYY").year();
      birthday.month = moment(req.body.birthdate, "MM-DD-YYYY").month() + 1;
      birthday.date = moment(req.body.birthdate, "MM-DD-YYYY").date();
      ...
      tasker.working_days = req.body.working_days.map(days => {
        days.hour.morning = days.hour.morning == 'true' ? true : false
        days.hour.afternoon = days.hour.afternoon == 'true' ? true : false
        days.hour.evening = days.hour.evening == 'true' ? true : false
        return days;
      });
    ...
    controller.registerChildCategory = function(req, res) {
      ...
      db.GetAggregation('category', [{
        ...
        if (err) {
          res.send({
            "status": "0",
            "errors": res.__("Check The Category Id Is Correct")
          });
        } else {
          res.send(category);
        }
                      ...
                      db.GetAggregation('task', [{
                        ...
                        if (err || bookings.length == 0) {
                          ...
                          for (var i = 0; i < bookings.length; i++) {
							...
							job.total_amount = ((bookings[i].invoice.amount.total + job.meterial_fee * currencies.value) - (bookings[i].invoice.amount.admin_commission * currencies.value)).toFixed(2) || '';
    ...
    controller.updatemobilecategory = function updatemobilecategory(req, res) {
      ...
      db.GetOneDocument('tasker', {
        ...
        if (err) {
          ...
          checkcattype(req.body.childid, function (response) {
            data.taskerskills.ratetype = req.body.ratetype ? req.body.ratetype : response;
    ...
    controller.updateproviderlanguage = function (req, res) {
        ...
        db.UpdateDocument('tasker', { '_id': request.userid }, { 'language': request.langcode }, {}, function (err, response) {
            if (err || response.nModified == 0) {
